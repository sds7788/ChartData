...
#     从包含两种不同结构JSON文件的目录中，创建一个统一的视觉问答（VQA）数据集。
#     此版本修改：
#     1. 静默处理所有错误：无论是JSON文件本身有问题（无法解析、缺少字段），还是其关联的图片文件不存在，
#        程序都将直接跳过，不打印任何警告或错误信息，也不将该条目计入最终数据集。
#     2. 保留了原有功能：将每个条目对应的 `source_data`, `task_type`, 和 `renderer` 完整地打包到最终数据集中。
#     3. 保留了原有功能：如果 `task_type` 字段缺失，则默认为 'single'。
#     """
#     # --- 1. 初始化和环境设置 ---
#     dataset_root_dir = os.path.dirname(dataset_filename)
#     if not os.path.exists(dataset_root_dir):
...
#                 chart_type = generation_info.get('chart_type')
#                 task_type = generation_info.get('task_type', 'single') 
#                 renderer = generation_info.get('renderer')
#                 generated_charts = data.get('generated_charts', [])
#                 source_data = data.get('source_data_annotated', {}) 
...
#                     if original_filename_in_json:
#                         target_filename = original_filename_in_json.replace('.html', '.png') if original_filename_in_json.endswith('.html') else original_filename_in_json
#                         image_to_find_path = os.path.join(output_dir, target_filename)
...
#             # **分支3：处理未知结构的JSON文件**
#             # 如果文件结构不匹配以上任何一种，则不执行任何操作，循环将自动处理下一个文件

#         except (json.JSONDecodeError, Exception):
#             # 捕获所有与此文件相关的异常（如JSON解析错误、键错误等）
#             # 并静默地继续处理下一个文件，不记录任何内容。
#             pass
...
#         print("-" * 30)
#         print("数据集创建成功！")
#         print(f"总共处理了 {image_id_counter} 张有效图片和 {len(dataset['annotations'])} 个有效问答对。")
#         print(f"图片已保存到 '{images_dir}' 目录。")
#         print(f"数据集信息已保存到 '{dataset_filename}' 文件。")
#     else:
#         print("-" * 30)
#         print("数据集创建失败。")
#         print("没有找到任何有效的数据条目（请检查JSON文件内容和图片文件是否存在）。")
...
#     # 提示：在Windows路径中使用双反斜杠'\\'或正斜杠'/'可以避免转义字符问题
#     create_vqa_dataset(
#         output_dir='D:\\桌面\\数据存储\\output重换新生', 
#         images_dir='D:\\桌面\\数据存储\\dataset(2,20k)\\images', 
#         dataset_filename='D:\\桌面\\数据存储\\dataset(2,20k)\\dataset.json'
#     )
...
def create_vqa_dataset(output_dir, images_dir, dataset_filename):
    ...
    从包含两种不同结构JSON文件的目录中，创建一个统一的视觉问答（VQA）数据集。
    此版本修复了一个关键Bug：
    1.  **修复**：通过调整操作顺序，确保了 image 和 annotation 的添加是原子操作，
        彻底避免了因 `answer` 字段异常而导致最终数量不匹配的问题。
    2.  保留了静默处理所有文件级错误的功能。
    3.  保留了原有功能：打包 `source_data`, `task_type`, `renderer` 并处理默认 `task_type`。
    """
    # --- 1. 初始化和环境设置 ---
    dataset_root_dir = os.path.dirname(dataset_filename)
    if not os.path.exists(dataset_root_dir):
    ...
    for filename in tqdm(files_to_process, desc="打包数据集"):
        ...
        try:
            ...
            if 'generated_charts' in data and 'qa_pair' in data:
                ...
                if not all([question, answer is not None, chart_type, generated_charts]):
                    continue
                    ...
                    if original_filename_in_json:
                        ...
                        if os.path.exists(image_to_find_path):
                            try:
                                # ############## 核心修复点 ##############
                                # 1. 先准备好所有可能出错的数据
                                answer_str = str(answer) # 如果answer有问题，这里会抛出异常
                                new_image_id = image_id_counter + 1
                                new_question_id = question_id_counter + 1

                                # 2. 如果准备数据没问题，再进行文件操作和修改列表
                                new_image_filename = f"{new_image_id:05d}.png"
                                new_image_path = os.path.join(images_dir, new_image_filename)
                                shutil.copy2(image_to_find_path, new_image_path)
                                ...
                                # 3. 最后，原子化地更新所有内容
                                image_id_counter = new_image_id
                                question_id_counter = new_question_id
                                ...
                                dataset['annotations'].append({
                                    ...
                                    "question": question, "answers": [{"answer": answer_str}],
                                    "source_data": source_data
                                })
                            except Exception:
                                # 如果在准备或处理单个图片时出错，静默跳过这个图片
                                pass
            ...
            # **分支2：处理“单一图表实体”类型的JSON**
                ...
                if not all([question, answer is not None, chart_type, original_image_filename]):
                    continue
                ...
                if os.path.exists(image_to_find_path):
                    try:
                        ...
                        dataset['images'].append({
                            "id": image_id_counter, "file_name": new_image_filename,
                            "height": height, "width": width, "chart_type": chart_type,
                            "complexity_level": 0, "task_type": task_type,
                            "renderer": renderer
                        })
                        dataset['annotations'].append({
                            "question_id": question_id_counter, "image_id": image_id_counter,
